...
Proof.
  ...
  pose proof @path_grow _ Lft nd true false.
...
Definition form_2 P Q :=
  (|- (CWhile true CSkip) : (P /\ true) => (Q /\ true)).
...
Lemma form_2_rules nd P Q :
  ...
  rg_rule nd = Rule RGP.HL_WhileTrue.
Proof.
  pose proof @form_1_rules nd (P /\ true)%A (Q /\ true)%A.
  intros. applys~ H.
  apply valid_and. splits~. unfolds. intros. now simpls.
Qed.
...
Proof.
  ...
  rewrite Conc in H. unfolds form_3.
  destruct~ r; inverts H.
Qed.
...
Lemma strong_csq nd P Q c :
  ...
  List.In nd' (rg_prems nd).
Proof.
  intros Conc Rule TautP TautQ.
  pose proof rg_wf nd. rewrite Rule in H.
  inverts H. rewrite Conc in H1.
  unfold form_1 in H1. inverts H1. sort.
  (* Break down prems list into 3 items *)
  destruct (rg_prems nd). discriminate.
  simpls. destruct l. discriminate. simpls.
  destruct l. discriminate.
  ...
  - specializes lift_valid r1.
    rewrite <- H3 in lift_valid.
    destruct (rg_rule r1) eqn:E.
    pose proof rg_wf r1. rewrite E in H.
    rewrite <- H3 in H. inverts H.
    unfolds in lift_valid.
    introv. specializes TautQ m I.
    specializes lift_valid m I.
    simpls. contradict TautQ.
    now apply lift_valid.
Qed.

Lemma form_2_while nd P Q :
  rg_conc nd = form_2 P Q ->
  rg_rule nd = Rule RGP.HL_WhileTrue ->
  (|= P)%V ->
  (|= ~Q)%V ->
...
Lemma form_step nd P Q :
  ...
  List.In nd' (rg_prems nd).
Proof.
  intros F HP HQ. destruct F as [F|F].
  - pose proof form_1_rules _ F HP.
    destruct H.
    + pose proof strong_csq nd F H HP HQ. exists* H0.
      exists nd' P' Q'. splits~. unfolds. left~.
    + pose proof form_1_while nd F H HP HQ. exists* H0.
      exists nd' P' Q'. splits~. right~.
  - pose proof form_3_rules _ F.
    destruct H.
    + pose proof strong_csq nd F H HP HQ. exists* H0.
      exists nd' P' Q'. splits~. unfolds. right~.
    + pose proof form_3_seq nd F H HP HQ. exists* H0.
      exists nd' P' Q'. splits~. left~.
Qed.
...
Proof.
  ...
  induction n.
  { intros. now exists nd P Q (single_path nd). }
  destruct IHn as (nd'&P'&Q'&p&F'&HP'&HQ'&Fst&Lst&L). sort.
  pose proof form_step F' HP' HQ' as (nd''&P''&Q''&HP''&HQ''&F''&In).
  sort.
  exists nd'' P'' Q''.
  assert (Path : is_path (nd'::nd''::[])) by easy.
  pose proof path_appending p (exist _ _ Path) Lst as Path'.
