...
Definition pair (v1 v2 : val) := (val_pair v1 v2).
...
Inductive evalunop : prim -> val -> val -> Prop :=
  ...
  | evalunop_pair_fst : forall v1 v2,
      evalunop val_pair_fst (val_pair v1 v2) v1
  | evalunop_pair_snd : forall v1 v2,
      evalunop val_pair_snd (val_pair v1 v2) v2.
...
Inductive evalbinop : val -> val -> val -> val -> Prop :=
  ...
  | evalbinop_mod : forall n1 n2,
      n2 <> 0 ->
      evalbinop val_mod (val_int n1) (val_int n2) (val_int (Z.modulo n1 n2))
  ...
  | evalbinop_pair_make : forall v1 v2,
      evalbinop val_pair_make v1 v2 (val_pair v1 v2).
...
Lemma hexists_detach : forall A (H : A->hprop) h P,
  (\exists (x : A), H x) h ->
  ((\exists (y : A), H y) h -> P) ->
  P.
Proof using. intros. apply H1 in H0. assumption. Qed.
...
Lemma triple_mod : forall n1 n2,
  ...
  triple (val_mod n1 n2)
    \[]
    (fun r => \[r = val_int (Z.modulo n1 n2)]).
...
Lemma triple_pair_make : forall v1 v2,
  triple (val_pair_make v1 v2)
    \[]
    (fun r => \[r = val_pair v1 v2]).
Proof using. intros. applys* triple_binop. applys* evalbinop_pair_make. Qed.

Lemma triple_pair_fst : forall v1 v2,
  triple (val_pair_fst (val_pair v1 v2))
    \[]
    (fun r => \[r = v1]).
Proof using. intros. applys* triple_unop. applys* evalunop_pair_fst. Qed.

Lemma triple_pair_snd : forall v1 v2,
  triple (val_pair_snd (val_pair v1 v2))
    \[]
    (fun r => \[r = v2]).
Proof using. intros. applys* triple_unop. applys* evalunop_pair_snd. Qed.
...
Hint Resolve triple_add triple_div triple_neg triple_opp triple_eq
   ...
   triple_ge triple_gt triple_ptr_add triple_ptr_add_nat 
   triple_pair_make triple_pair_fst triple_pair_snd : triple.
...
Notation "'`pair'" :=
  (trm_val (val_prim val_pair_make))
  (in custom trm at level 0) : trm_scope.

Notation "'fst'" :=
  (trm_val (val_prim val_pair_fst))
  (in custom trm at level 0) : trm_scope.

Notation "'snd'" :=
  (trm_val (val_prim val_pair_snd))
  (in custom trm at level 0) : trm_scope.

(*Notation "v1 , v2" :=
  (val_pair v1 v2)
  (in custom trm at level 100) : trm_scope.*)
...
Module ExPair. Import Vars.

Definition fstpair : val :=
  <{ fun 'p => 
       let 'v = ! 'p in
       let 'f = fst 'v in
       'f }>.

Definition sndpair : val :=
  <{ fun 'p => 
      let 'v = ! 'p in
      let 'f = snd 'v in
      'f }>.

Definition makepair : val :=
  <{ fun 'p 'q 'r => 
      let 'u = ! 'p in
      let 'v = ! 'q in
      let 'f = `pair 'u 'v in
...
Lemma triple_fstpair : forall (p : loc) (v1 v2 : val),
  triple (fstpair p)
    (p ~~> pair v1 v2)
    (fun r => \[r = v1] \* p ~~> pair v1 v2).
Proof using.
  xwp. xapp. xapp. xval. xsimpl*.
Qed.

Lemma triple_sndpair : forall (p : loc) (v1 v2 : val),
  triple (sndpair p)
    (p ~~> pair v1 v2)
    (fun r => \[r = v2] \* p ~~> pair v1 v2).
Proof using.
  xwp. xapp. xapp. xval. xsimpl*.
Qed.

Lemma triple_makepair : forall (p q r : loc) (v1 v2 _v : val),
  triple (makepair p q r)
    (p ~~> v1 \* q ~~> v2 \* r ~~> _v)
    (fun o => \[o = val_unit] \* p ~~> v1 \* q ~~> v2 \* r ~~> pair v1 v2).
Proof using.
  xwp. xapp. xapp. xapp. xapp. xval. xsimpl*.
Qed.

Lemma triple_writenext : forall (p q : loc) (v _v : val),
  triple (write_next p)
    (p ~~> pair q v \* q ~~> _v)
    (fun r => \[r = _v] \* p ~~> pair q v \* q ~~> v).
Proof using.
  xwp. repeat xapp. xval. xsimpl*.
Qed.
