...
\todo{Maybe the def of interpretation should be all sets which yield bot or true. I think that cleans things up maybe?
Alternatively, validity can be defined relative to a memory, e.g. $m \vDash P$. Alternatively still, memories can be defined as total and programs are assumed to be well-typed so that expressions never yield bot.}
\begin{displaymath}
\begin{array}{rcl}
...
\begin{mathpar}
    ...
    { \Eval(e,m)=v \neq \bot }
    { \Cfg(\Assn x = e, m) \sstepsto \Cfg(\Skip, \MemDef m[x->v]) }
    % Sequencing
    ...
    { \Eval(e,m) = \True}
    { \Cfg(\ITE e then c_1 else c_2, m) \sstepsto \Cfg(c_1, m)}
    % If false
    \and \inferrule*[right=SS-If-F]
    { \Eval(e,m) = \False}
    { \Cfg(\ITE e then c_1 else c_2, m) \sstepsto \Cfg(c_2, m)}
    % While true
...
\end{lemma}
\begin{proof}
    \todo{Proof by induction on length of execution.}
\end{proof}
...
\begin{mathpar}
    ...
    { }
    { \DER \Triple \Assn x = e : \Subst P[e/x] => P }
    \and
    ...
    { \DER \Triple c : P \wedge e => Q }
    { \DER \Triple \ITE e then c else c' : P \wedge e => Q }
    \and
    \inferrule*[right=HL-If-F]
    { \DER \Triple c' : P \wedge \neg e => Q }
    { \DER \Triple \ITE e then c else c' : P \wedge \neg e => Q }
    \\
    \inferrule*[right=HL-While-T]
    { \DER \Triple \Seq c; \While e do c : P \wedge e => Q }
    { \DER \Triple \While e do c : P \wedge e => Q \wedge \neg e }
    \and
    \inferrule*[right=HL-While-F]
    { }
    { \DER \Triple \While e do c : P \wedge \neg e => P \wedge \neg e }
\end{mathpar}
