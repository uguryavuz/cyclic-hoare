...
lemma ind (n : int) (S <: STRM{-L.M}) :
 equiv [ L.M(S).f ~ L.M(S).f :
   ={L.M.ctr, glob S, sum} /\
   0 <= L.M.ctr{1} /\
   0 <= L.M.ctr{2} /\
   L.M.ctr{1} = n
...
rcondt{2} 1; auto.
sp.
conseq (_ : ={L.M.ctr, glob S, sum} /\
            0 <= L.M.ctr{1} /\ 0 <= L.M.ctr{2} /\ L.M.ctr{1} = n ==> ={sum}).
auto.
admit. (* IH *)
...
lemma overall (S <: STRM{-L.M}) :
  equiv [ L.M(S).g ~ L.M(S).g :
      ={L.M.ctr, glob S} /\
        L.M.ctr{1} = L.lim
      ==>
      ={res}
...
lemma ind (n : int) (S <: STRM{-LimA.M, -LimB.M}) :
 equiv [ LimA.M(S).f ~ LimB.M(S).f : 
   ={LimA.M.ctr, LimB.M.ctr} /\
   0 <= LimA.M.ctr{1} /\
   0 <= LimB.M.ctr{2} /\
   LimA.M.ctr{1} + LimB.M.ctr{2} = n
   ==>
   ={res}
 ].
proof.
...
proc.
sp.
seq 1 1 : (={sum} /\ ={LimA.Lim.ctr, LimB.Lim.ctr} /\
   0 <= LimA.Lim.ctr{1} /\
   0 <= LimB.Lim.ctr{1} /\
   LimA.Lim.ctr{1} + LimB.Lim.ctr{1} = 0). 
...
call (_ : true). auto. progress. smt(). smt().
